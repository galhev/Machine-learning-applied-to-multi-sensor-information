...
testResultsMichael<-as.data.frame(matrix(list(),3,7))
testResultsRFMean<-list()
testResultsRF<-as.data.frame(matrix(list(),3,7))
testResultsRFConf<-as.data.frame(matrix(list(),3,7))
varImp<-as.data.frame(matrix(list(),3,7))
tuningModelsTrees<-as.data.frame(matrix(0,3,7))
tuningModelsVars<-as.data.frame(matrix(0,3,7))
...
j=1
for(param1 in 1:7){
      print (paste0("param1: ",colnames(df.train[param1])))
      names(tuningModelsTrees)[j]<-(paste0(colnames(df.train[param1])))
      names(tuningModelsVars)[j]<-(paste0(colnames(df.train[param1])))
      trainWithMissTrain<-df.train[,c(-param1)]
      trainWithMissValidation<-df.validation[,c(-param1)]
      numtreesSeq <- seq(60,180,40) 
...
colnames(tuningModelsTrees)[1:7]<-names(tuningModelsTrees)[1:7]
colnames(tuningModelsVars)[1:7]<-names(tuningModelsVars)[1:7]
...
for(param1 in 1:7){
      ...
      names(testResultsRF)[j]<-(paste0(colnames(df.train[param1])))
      j=j+1
    }
...
for(param1 in 1:7){
      testWithMiss<-df.test[,c(-param1)]
      michael.pred<-Michael_Algo(testWithMiss, colnames(df.train[param1]), colnames(df.train[param1]), colnames(df.train[param1]))
      testResultsMichael[[run,j]]<-Model_Test_Measure_With_Miss(michael.pred, "michael", testWithMiss, df.test.tags, TRUE, 1)
      names(testResultsMichael)[j]<-(paste0(colnames(df.train[param1])))
      j=j+1
    }
colnames(testResultsMichael)[1:7]<-names(testResultsMichael)[1:7]
...
for(x in 1:7){
  ...
  names(testResultsMichaelMean)[x]<-(paste0(colnames(df.train[x])))
  testResultsRFMean[[x]]<-(testResultsRF[[1,x]]+testResultsRF[[2,x]]+testResultsRF[[3,x]])/3
  names(testResultsRFMean)[x]<-(paste0(colnames(df.train[x])))
  }
...
write.csv(tuningModelsTrees,"C:/Users/user/Desktop/ThesisCode/OneMiss/TuningModels/tuningModelsTrees.csv")
write.csv(tuningModelsVars,"C:/Users/user/Desktop/ThesisCode/OneMiss/TuningModels/tuningModelsVars.csv")
write.csv(results,"C:/Users/user/Desktop/ThesisCode/OneMiss/results.csv")
