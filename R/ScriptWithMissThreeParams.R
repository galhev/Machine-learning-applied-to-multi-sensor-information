...
testResultsMichael<-as.data.frame(matrix(list(),3,35))
testResultsRFMean<-list()
testResultsRF<-as.data.frame(matrix(list(),3,35))
testResultsRFConf<-as.data.frame(matrix(list(),3,35))
varImp<-as.data.frame(matrix(list(),3,35))
tuningModelsTrees<-as.data.frame(matrix(0,3,35))
tuningModelsVars<-as.data.frame(matrix(0,3,35))
...
for(param1 in 1:7){
  if((param1+1)<7){
    for(param2 in (param1+1):7){
      if((param2+1)<=7){
        for(param3 in (param2+1):7){
                print (paste0("param1: ",colnames(df.train[param1]), " ,param2: ", colnames(df.train[param2])," ,param3: ", colnames(df.train[param3])))
                names(tuningModelsTrees)[j]<-(paste0(colnames(df.train[param1]), " ", colnames(df.train[param2]), " ", colnames(df.train[param3])))
                names(tuningModelsVars)[j]<-(paste0(colnames(df.train[param1]), " ", colnames(df.train[param2]), " ", colnames(df.train[param3])))
                
                trainWithMissTrain<-df.train[,c(-param1, -param2, -param3)]
                trainWithMissValidation<-df.validation[,c(-param1, -param2, -param3)]
...
colnames(tuningModelsTrees)[1:35]<-names(tuningModelsTrees)[1:35]
...
for(param1 in 1:7){
  if((param1+1)<7){
    for(param2 in (param1+1):7){
      if((param2+1)<=7){
        for(param3 in (param2+1):7){
            trainWithMiss<-df.train[,c(-param1, -param2, -param3)]
            RF <- createRF(trainWithMiss, df.train.tags, tuningModelsTrees[run,j], tuningModelsVars[run,j], 0)
            models[[j]]<-RF
            names(models)[j]<-(paste0(colnames(df.train[param1]), " ", colnames(df.train[param2]), " ", colnames(df.train[param3])))
            j=j+1
        }
      }
    }
...
for(param1 in 1:7){
  if((param1+1)<7){
    for(param2 in (param1+1):7){
      if((param2+1)<=7){
        for(param3 in (param2+1):7){
          testWithMiss<-df.test[,c(-param1, -param2, -param3)]
          RF<-models[[j]]
          ...
          names(testResultsRF)[j]<-(paste0(colnames(df.train[param1]), " ", colnames(df.train[param2]), " ", colnames(df.train[param3])))
          j=j+1
        }
      }
    }
  ...
  if((param1+1)<7){
    for(param2 in (param1+1):7){
      if((param2+1)<=7){
        for(param3 in (param2+1):7){
          testWithMiss<-df.test[,c(-param1,-param2, -param3)]
          michael.pred<-Michael_Algo(testWithMiss, colnames(df.train[param1]),colnames(df.train[param2]),colnames(df.train[param3]))
          testResultsMichael[[run,j]]<-Model_Test_Measure_With_Miss(michael.pred, "michael", testWithMiss, df.test.tags, TRUE, 1)
          names(testResultsMichael)[j]<-(paste0(colnames(df.train[param1]), " ", colnames(df.train[param2]), " ", colnames(df.train[param3])))
          j=j+1
        }
      }
    }
...
for(x in 1:35){
  ...
  names(testResultsMichaelMean)[x]<-(paste0(colnames(testResultsMichael[x])))
  testResultsRFMean[[x]]<-(testResultsRF[[1,x]]+testResultsRF[[2,x]]+testResultsRF[[3,x]])/3
  names(testResultsRFMean)[x]<-(paste0(colnames(testResultsMichael[x])))
}
...
write.csv(tuningModelsTrees,"C:/Users/user/Desktop/ThesisCode/ThreeMiss/TuningModels/tuningModelsTrees.csv")
write.csv(tuningModelsVars,"C:/Users/user/Desktop/ThesisCode/ThreeMiss/TuningModels/tuningModelsVars.csv")
write.csv(results,"C:/Users/user/Desktop/ThesisCode/ThreeMiss/results.csv")
